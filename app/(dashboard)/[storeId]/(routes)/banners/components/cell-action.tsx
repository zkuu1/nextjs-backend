...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onDelete = async () => {
    ...
    try {
      setLoading(true);
      console.log("Sending DELETE request to:", `/api/${params.storeId}/banners/${data.id}`);
      const response = await axios.delete(`/api/${params.storeId}/banners/${data.id}`);
      console.log("Delete response:", response.data);

      router.refresh(); // Refresh data tabel
      console.log("Refreshing router...");
      router.push(`/${params.storeId}/banners`); // Redirect kembali ke halaman banners
